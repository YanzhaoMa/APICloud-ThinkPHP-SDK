...
class HttpModel 
{
	...
	private function exec($url,$method='GET',$body='')
	{
	    ...
	    curl_setopt($this->curl, CURLOPT_HTTPHEADER , array("X-HTTP-Method-Override: $method"));
	    ...
	    curl_setopt($this->curl, CURLOPT_FOLLOWLOCATION, $this->location); // 使用自动跳转
	    curl_setopt($this->curl, CURLOPT_AUTOREFERER, $this->autoreferer); // 自动设置Referer
	    //curl_setopt($this->curl, CURLOPT_POST, 0); // 发送一个常规的Post请求
	    curl_setopt($this->curl, CURLOPT_POSTFIELDS, $body); // Post提交的数据包
	    curl_setopt($this->curl, CURLOPT_TIMEOUT, $timeout); // 设置超时限制防止死循环
	    curl_setopt($this->curl, CURLOPT_HEADER, $this->header); // 显示返回的Header区域内容
	    curl_setopt($this->curl, CURLOPT_RETURNTRANSFER, $this->returntransfer); // 获取的信息以文件流的形式返回
	    $totalline = curl_exec($this->curl); // 执行操作
	    if (curl_errno($this->curl)) {
	    	echo 'Errno'.curl_error($this->curl);//捕抓异常
	    }
