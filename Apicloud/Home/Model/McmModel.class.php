...
class McmModel extends ApicloudModel
{
	...
	{
		return $this->http->post(self::SERVERAPIURL.'mcm/api/'.$name,$obj);
	}
	function objFindAll($name,$filter) //获取所有对象filter数据类型:json(可连接参数:fields,where,limit,skip,order,include,includefilter)
	{
		return $this->http->get(self::SERVERAPIURL.'mcm/api/'.$name.'?filter='.$filter); 
	}
	function objGet($name,$id='') //获取对象ById
	{
		return $this->http->get(self::SERVERAPIURL.'mcm/api/'.$name.'/'.$id); 
	}
	function objPut($name,$id,$obj) //更新对象ById
	{
		return $this->http->put(self::SERVERAPIURL.'mcm/api/'.$name.'/'.$id,$obj);
	}
	function objDelete($name,$id)  //删除对象ById
	{
		return $this->http->delete(self::SERVERAPIURL.'mcm/api/'.$name.'/'.$id);
	}
	function objCount($name)  //统计对象数量
	{
		return $this->http->count(self::SERVERAPIURL.'mcm/api/'.$name.'/count');
	}
	function objExists($name,$id) //判断对象是否存在ById
	{
		return $this->http->exists(self::SERVERAPIURL.'mcm/api/'.$name.'/'.$id.'/exists');
	}
	...
	function relationGet($name,$id,$relationName){  //获取关联对象
		return $this->http->get(self::SERVERAPIURL.'mcm/api/'.$name.'/'.$id.'/'.$relationName);
	}
	function relationPost($name,$id,$relationName,$property){  //创建关联对象
		return $this->http->post(self::SERVERAPIURL.'mcm/api/'.$name.'/'.$id.'/'.$relationName,$property);
	}
	function relationGetCount($name,$id,$relationName){  //统计关联对象数量
		return $this->http->get(self::SERVERAPIURL.'mcm/api/'.$name.'/'.$id.'/'.$relationName.'/count');
	}
	function relationDelete($name,$id,$relationName){  //删除所有关联对象
		return $this->http->delete(self::SERVERAPIURL.'mcm/api/'.$name.'/'.$id.'/'.$relationName);
	}
	...
	{
		$this->http->headerAdd(array("authorization: $access_token"));
		return $this->http->post(self::SERVERAPIURL.'mcm/api/user/logout');
	}
	...
	function userCount($filter){  //获取所有用户数量:json(可连接参数:fields,where,limit,skip,order,include,includefilter)
		return $this->http->get(self::SERVERAPIURL.'mcm/api/user/count?filter='.$filter);
	}
	function userFindAll($filter){  //获取所有用户数据:json(可连接参数:fields,where,limit,skip,order,include,includefilter)
		return $this->http->get(self::SERVERAPIURL.'mcm/api/user?filter='.$filter);
	}
	...
	{
		return $this->http->get(self::SERVERAPIURL.'mcm/api/role/count/?filter='.$filter); 
	}
	...
	function roleFindAll($filter){  //获取所有对象filter数据类型:json(可连接参数:fields,where,limit,skip,order,include,includefilter)
		return $this->http->get(self::SERVERAPIURL.'mcm/api/role?filter='.$filter);
	}
